! equations_ppf_CDE.f90
    ...
    use IniFile
    Type(TIniFile) :: Ini
    log_phi_i = Ini_Read_Double_File(Ini,'log_phi_i', 0.d0)
    chi_i = Ini_Read_Double_File(Ini,'chi_i', 0.d0)
    log_lambda_chi = Ini_Read_Double_File(Ini,'log_lambda_chi', 0.d0)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	real(16) :: Omegam0,Omegar0,Omegak0,lambdachit,phitinitial,chitinitial
...
!---CDE Start
	...
	grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass+grhov*a2**2
	...
	Omegak0 = real(grhok /(3.0d0*H0inMpcinsec**2),16)

	phitinitial = real(10.0d0**log_phi_i,16)
	chitinitial = real(chi_i,16)	
	lambdachit = real(10.0d0**log_lambda_chi,16)
	...
	Hta2value = Hta2(real(a,16), Omegam0, Omegar0, Omegak0, lambdachit, phitinitial, chitinitial)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	function Hta2(ainput,Omegam0input,Omegar0input,Omegak0input, &
		 & lambdachitinput,phitinitialinput,chitinitialinput)

	implicit none

	real(16) :: ainput,Omegam0input,Omegar0input,Omegak0input, &
		        &   lambdachitinput,phitinitialinput,chitinitialinput
	real(16) :: Hta2
	

	real(16) :: Omegam0,Omegar0,Omegak0,lambdachit,phitinitial,chitinitial
	common /inputparametersofHta2/ Omegam0,Omegar0,Omegak0,lambdachit,phitinitial,chitinitial
	...
	real(16) :: Omegam0flag,Omegar0flag,Omegak0flag,lambdachitflag,phitinitialflag, chitinitialflag
	common /flags/ Omegam0flag,Omegar0flag,Omegak0flag,lambdachitflag,phitinitialflag, chitinitialflag
	...
	common / valueoffield/phitpi, chitpi
	common / valueofinitial/phiti, chiti
	...
	Omegak0 = Omegak0input	
	lambdachit = lambdachitinput
	phitinitial = phitinitialinput
	chitinitial = chitinitialinput

	if ((Omegam0 == Omegam0flag) .and. (Omegar0 == Omegar0flag) .and. (Omegak0 == Omegak0flag) .and. &
	...
	rhoktinitial = 3.0q0*Omegak0*aitoa0approx**(-2.0q0)
	...
	lambdaphit = 12.0q0*(1.0q0 - Omegam0 - Omegar0 - Omegak0)/phitinitial**4	
	...
	phitpinitial = (-4.0q0*lambdaphit*phitinitial**3.0q0)/ &
	                  ...
	                  &  exp(4.0q0*Neinitial) + lambdaphit*phitinitial**4.0q0 + lambdachit*chitinitial**4.0q0)

	chitpinitial = (-4.0q0*lambdachit*chitinitial**3.0q0)/ &
    				 &  ((4.0q0*rhomtinitial)/exp(3.0q0*Neinitial) - (12.0q0*rhoktinitial)/exp(2.0q0*Neinitial) + (4.0q0*rhortinitial)/ &
    				 &  exp(4.0q0*Neinitial) + lambdaphit*phitinitial**4.0q0 + lambdachit*chitinitial**4.0q0)

	Htinitial = ((-(((4.0q0*(exp(Neinitial)*rhomtinitial - 3.0q0*rhoktinitial*exp(2.0q0*Neinitial) + rhortinitial))/exp(4.0q0*Neinitial) + &
	...
	if ((isnan(lambdaphit)) .or. (abs(Ne0 - Ne0approx) > 5.0q0)) then
		...
		omegam_bao = 1.0q0 - Omegak0 - lambdaphit*phit0**4/12.0q0	
	...
	Hta2 = (arrayHt(i) + ((arrayHt(i+1) - arrayHt(i))/(arraya(i+1) - arraya(i)))*(log(ainput) - arraya(i)))*ainput**2
...
!---CDE Start
    ...
    EV%w_ix = neq+1			
    neq=neq+4						!neq+2 	 is only for one \phi 	=> 	 neq+4 	is for \phi + \chi
    maxeq=maxeq+4    				!maxeq+2 is only for one \phi 	=> 	 maxeq+4  is for \phi + \chi
...
!---CDE Start
	real(dl) adotoa_twofield, dtauda, dgrhophi, dgrhochi, dgqphi, dgqchi, qphi, qchi, clxphi, clxchi
	real(16) phitpi, phiti, chitpi, chiti, lambdaphit, lambdachit, Omegam0,Omegar0,Omegak0,phitinitial,chitinitial
	common /valueoffield/phitpi,chitpi
	common /valueofinitial/phiti,chiti
	common /lambdaphit/lambdaphit
	common /inputparametersofHta2/lambdachit, Omegam0,Omegar0,Omegak0,phitinitial,chitinitial						
!---CDE End    
...
!---CDE Start
    ...
    adotoa_twofield = 1/(a*dtauda(a))
    
    grho = 3*adotoa_twofield*adotoa_twofield
        
    !gpres=(grhog_t+grhor_t)/3+grhov_t*w_eff

 	gpres = (grhog_t+grhor_t)/3 + 0.5d0*(adotoa*phitpi)*(adotoa*phitpi)-0.25d0*a2*norm_V*lambdaphit*H0_to_Mpl_in_GeV**2*phiti**4._dl & 	
        						+ 0.5d0*(adotoa*chitpi)*(adotoa*chitpi)-0.25d0*a2*norm_V*lambdachit*H0_to_Mpl_in_GeV**2*chiti**4._dl
!---CDE End
...
!---CDE 
	...
	adotoa=sqrt(grho/3)		!---grhok has been included before							

!--- \phi	!---Equations B4 of arxiv 1908.06995		with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a 
     dgrhophi = adotoa*phitpi*k*qphi + a2*clxphi*norm_V*lambdaphit*H0_to_Mpl_in_GeV**2*phiti**3		
     dgqphi = k*adotoa*phitpi*clxphi			!---Equations B6 of arxiv 1908.06995

!--- \chi	!---Equations B4 of arxiv 1908.06995		with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a
     dgrhochi = adotoa*chitpi*k*qchi + a2*clxchi*norm_V*lambdachit*H0_to_Mpl_in_GeV**2*chiti**3		    		
     dgqchi = k*adotoa*chitpi*clxchi		!---Equations B6 of arxiv 1908.06995

     dgrho = dgrho + dgrhophi + dgrhochi
     dgq = dgq + dgqphi + dgqchi 	    
!---CDE End
...
!---CDE Start      
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        i_qg=5,i_qr=6,i_vb=7,i_pir=8, i_eta=9, i_aj3r=10, i_clxphi=11, i_vphi=12, i_clxchi=13, i_vchi=14
    integer, parameter :: i_max = i_vchi
    real(dl) initv(6,1:i_max), initvec(1:i_max)
!---CDE End
...
!---CDE Start   
    ...
    !---\delta \phi
    y(EV%w_ix)   = InitVec(i_clxphi)	

	!---\delta \phi' = k \delta \Psi
	y(EV%w_ix+1) = InitVec(i_vphi)			

	!---\delta \chi
    y(EV%w_ix+2) = InitVec(i_clxchi)	

	!---\delta \chi' = k \delta \Gamma 
    y(EV%w_ix+3) = InitVec(i_vchi)   			
!---CDE End   
...
!---CDE Start
    ...
    real(dl) clxphi, qphi,  E2, dopacity
	real(dl) dgqphi, dgrhophi, dgqchi, dgrhochi, qchi, clxchi, adotoa_twofield ,dtauda
	real(16) phitpi, phiti, chitpi, chiti, lambdaphit, lambdachit, Omegam0,Omegar0,Omegak0,phitinitial,chitinitial
	common /valueoffield/ phitpi, chitpi
	common /valueofinitial/ phiti, chiti
	common /lambdaphit/ lambdaphit
...
!---CDE End
    ...
    grhog_t=grhog/a2
!---CDE Start    
    !if (is_cosmological_constant) then				!---redundant lines
    !    grhov_t=grhov*a2
    !    w_eff = -1_dl
    !else
    !    !ppf
...
!---CDE Start
	...
	adotoa_twofield = 1._dl/(a*dtauda(a))
	
	grho = 3._dl*adotoa_twofield*adotoa_twofield
    ...
    if (CP%flat) then
        ...
        adotoa=sqrt(grho/3._dl)							!---grhok has been included before        				
...
!--- \phi	!---Equations B4 of arxiv 1908.06995		with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a 
     dgrhophi = adotoa*phitpi*qphi + a2*clxphi*norm_V*lambdaphit*H0_to_Mpl_in_GeV**2*phiti**3		
     dgqphi = k*adotoa*phitpi*clxphi			!---Equations B6 of arxiv 1908.06995

!--- \chi	!---Equations B4 of arxiv 1908.06995		with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a
     dgrhochi = adotoa*chitpi*qchi + a2*clxchi*norm_V*lambdachit*H0_to_Mpl_in_GeV**2*chiti**3		    		
     dgqchi = k*adotoa*chitpi*clxchi			!---Equations B6 of arxiv 1908.06995
...
!---CDE Start
        EV%OutputTransfer(Transfer_delta_phi) = clxphi
        EV%OutputTransfer(Transfer_delta_chi) = clxchi
        EV%OutputTransfer(Transfer_delta_phi_prime) = qphi
        EV%OutputTransfer(Transfer_delta_chi_prime) = qchi
!---CDE End          
    end if
...
!---CDE Start        
        ...
        gpres=gpres + 0.5d0*(adotoa*phitpi)*(adotoa*phitpi)-0.25d0*a2*norm_V*lambdaphit*H0_to_Mpl_in_GeV**2*phiti**4._dl & 	
        			+ 0.5d0*(adotoa*chitpi)*(adotoa*chitpi)-0.25d0*a2*norm_V*lambdachit*H0_to_Mpl_in_GeV**2*chiti**4._dl
...
!---CDE Start
	!---qphi is \delta \phi' = k \delta \Psi
	ayprime(EV%w_ix) = qphi	

	!---Equations B1 of arxiv 1908.06995; 	with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a
	ayprime(EV%w_ix+1) = -(z*phitpi + 2*qphi/k)*adotoa - (k*k + 3._dl*a2*norm_V*lambdaphit*H0_to_Mpl_in_GeV**2*phiti**2)*clxphi/k			
	
	!---qchi is \delta \chi' = k \delta \Gamma
	ayprime(EV%w_ix+2) = qchi		
	
	!---Equations B1 of arxiv 1908.06995; 	with d/dtau = dN/dtau * d/dN = adot/a * d/dN = \H d/dN , dN = d(ln a) = da/a
	ayprime(EV%w_ix+3) = -(z*chitpi + 2*qchi/k)*adotoa - (k*k + 3._dl*a2*norm_V*lambdachit*H0_to_Mpl_in_GeV**2*chiti**2)*clxchi/k
!---CDE End 
...
!---CDE Start
	real(dl) adotoa_twofield, dtauda
	real(16) phitpi, phiti, chitpi, chiti, lambdaphit, lambdachit, Omegam0,Omegar0,Omegak0,phitinitial,chitinitial
	common /valueoffield/phitpi,chitpi
	common /valueofinitial/phiti,chiti
	common /lambdaphit/lambdaphit
...
!---CDE Start    
    ...
    if (CP%flat) then
        ...
        cothxor=1._dl/tanfunc(tau/CP%r)/CP%r
        !adotoa=sqrt((grho+grhok)/3._dl)		!---grhok has been included before
        adotoa=sqrt(grho/3._dl)
    end if
!---CDE End
